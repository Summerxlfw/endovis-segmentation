#!/usr/bin/env python3
"""Create multi-task config files for MMSegmentation."""

import os

BASE_DIR = '/home/summer/endovis/mmsegmentation/configs/endovis'

# Multi-task config template - no base model inheritance
MULTITASK_TEMPLATE = '''_base_ = [
...
def main():
    ...
    for fold in range(4):
        config_path = os.path.join(BASE_DIR, f'endovis2017_multitask_fold{fold}.py')
        content = MULTITASK_TEMPLATE.format(fold=fold)
        with open(config_path, 'w') as f:
            f.write(content)
        print(f"Created: {config_path}")

    print("\nAll 4 multi-task config files created successfully!")
