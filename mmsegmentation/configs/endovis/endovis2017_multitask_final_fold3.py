...
model = dict(
    ...
    # Binary head: SHSA + 提高权重 + Dice Loss
    decode_head_binary=dict(
        ...
        align_corners=False,
        loss_decode=[
            dict(type='CrossEntropyLoss', use_sigmoid=False, loss_weight=1.5),
...
# Learning rate scheduler - 120k iterations
param_scheduler = [
    dict(type='LinearLR', start_factor=0.1, by_epoch=False, begin=0, end=2000),
    dict(type='PolyLR', eta_min=1e-6, power=0.9, begin=2000, end=120000, by_epoch=False)
]

# Training settings - 120k iterations
...
work_dir = './work_dirs/multitask/endovis2017_multitask_final_fold3'
