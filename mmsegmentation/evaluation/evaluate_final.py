#!/usr/bin/env python
"""Evaluate final multi-task model with Dice Loss."""
...
def main():
    print('=' * 70)
    print('最终优化模型评估 (120k iterations, CE+Dice Loss)')
    print('=' * 70)

    results = []
    for fold in range(4):
        config = f'/home/summer/endovis/mmsegmentation/configs/endovis/endovis2017_multitask_final_fold{fold}.py'
        ckpt = f'/home/summer/endovis/mmsegmentation/work_dirs/multitask/endovis2017_multitask_final_fold{fold}/iter_120000.pth'
        data_root = f'/home/summer/endovis/data/multitask/endovis2017_multitask_fold{fold}'

        if not os.path.exists(ckpt):
            print(f'Fold {fold}: checkpoint not found at {ckpt}')
            continue
    ...
    print('与单任务对比')
    print('=' * 70)
    print(f'{"任务":<10} {"单任务":<12} {"最终多任务":<12} {"差异":<10} {"结果":<10}')
    print('-' * 54)
    ...
    # 与综合多任务对比 (100k)
    combined_binary = 92.29
    combined_parts = 52.03
    combined_type = 26.46

    print()
    print('=' * 70)
    print('与综合多任务(100k)对比')
    print('=' * 70)
    print(f'{"任务":<10} {"综合100k":<12} {"最终120k":<12} {"提升":<10}')
    print('-' * 44)
    print(f'{"Binary":<10} {combined_binary:.2f}%{"":>6} {avg_binary:.2f}%{"":>6} {avg_binary-combined_binary:+.2f}%')
    print(f'{"Parts":<10} {combined_parts:.2f}%{"":>6} {avg_parts:.2f}%{"":>6} {avg_parts-combined_parts:+.2f}%')
    print(f'{"Type":<10} {combined_type:.2f}%{"":>6} {avg_type:.2f}%{"":>6} {avg_type-combined_type:+.2f}%')
